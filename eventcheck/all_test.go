...
func TestQueuedEventResult(t *testing.T) {
	e := &tdag.TestEvent{}

	we := queuedcheck.NewTask(e)
	assert.Panics(t, func() {
		err := we.Result()
		if err != nil {
			return
	...
	we.SetResult(errors.New("some error"))
	assert.Panics(t, func() { we.SetResult(errors.New("another error")) })

	assert.Equal(t, errors.New("some error"), we.Result())
}
